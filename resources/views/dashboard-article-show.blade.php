...
<x-app-layout>
    ...
    @section('content')
        ...
        @endif
        <div class="max-w-[1220px] w-[100%] shadow-sm bg-white m-auto mt-[32px] rounded-lg p-[16px] flex flex-col" data-drag-container data-article-id="{{ $article->id }}">
            ...
            <div class="flex flex-row font-medium p-[5px] text-gray-700 border-b-2 pb-[8px] border-gray-200 mb-4">
                <div class="w-[100%] flex flex-row justify-end gap-[5px]">
                    <a class="w-[30%] h-[32px] flex justify-center items-center p-[12px] rounded-md bg-gray-500 text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-500"  href="{{ route('dashboard-articles') }}">Vissza</a>
                    <a class="w-[30%] h-[32px] flex justify-center items-center p-[12px] rounded-md bg-blue-500 text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500"  href="{{ route('dashboard-articles.create', ['slug' => $article->slug]) }}">+Elem hozzáadás</a>
                </div>
            </div>
            ...
            @foreach($article->articleBlocks as $articleBlock)
                <div class="flex flex-row font-medium p-[5px] text-gray-700 pt-[8px] pb-[8px] border-gray-200 hover:bg-gray-200 cursor-move transition-all duration-200" 
                     data-article-block="{{ $articleBlock->id }}" 
                     data-drag-item 
                     data-block-id="{{ $articleBlock->id }}">
                    <h3 class="w-[8%] font-bold text-center">{{ $articleBlock->position }}</h3>
                    <h3 class="w-[12%]">{{ $articleBlock->type->name }}</h3>
                    <div class="w-[30%]">
                        @if($articleBlock->type->name === 'image')
                            @php
                            ...
                            @endif
                        @else
                            <h3>{{ Str::limit($articleBlock->content, 50) }}</h3>
                        @endif
                    </div>
                    <h3 class="w-[12%] text-sm">{{ $articleBlock->created_at->format('Y-m-d H:i') }}</h3>
                    <h3 class="w-[12%] text-sm">{{ $articleBlock->updated_at->format('Y-m-d H:i') }}</h3>
                    <div class="w-[26%] flex flex-row gap-[5px]">
                        <a class="w-[45%] flex h-[32px] justify-center items-center rounded-md bg-blue-500 text-sm font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500"  href="/dashboard-articles/{{$article->slug}}/edit/{{$articleBlock->id}}">Szerkesztés</a>
                        <form class="w-[45%]" method="POST" action="{{ route('dashboard-articles.destroy', ['slug' => $article->slug, 'id' => $articleBlock->id]) }}" 
                              data-crud="delete" 
                              data-confirm="true" 

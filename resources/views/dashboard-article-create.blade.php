<x-app-layout>
    ...
    @section('content')
        <div class="max-w-[1220px] w-[100%] shadow-sm bg-white m-auto mt-[32px] rounded-lg p-[16px] flex flex-col">
            ...
            <form id="article-create-form" method="POST" action="{{ route('dashboard-articles.store', ['slug' => $article->slug]) }}" 
                  data-crud="create" 
                  data-loading-title="Létrehozás..." 
                  data-loading-text="Kérjük várjon..." 
                  data-success-title="Sikeres!" 
                  data-redirect="{{ route('dashboard-articles.show', ['slug' => $article->slug]) }}">
                @csrf
                ...
                <div class="h-[50px]">
                    ...
                    <select name="type_id" id="type_id" class="w-[30%] border-2" required onchange="toggleContentField()">
                        ...
                        @foreach ($types as $type)
                            <option value="{{ $type->id }}" data-has-attributes="{{ $type->articleTypeAttributes ? 'true' : 'false' }}" data-type-name="{{ $type->name }}">
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div id="reflection-dropdown-section" class="hidden mt-4">
                    ...
                    <select name="reflection_id" id="reflection_id" class="w-[30%] border-2">
                        ...
                        @foreach ($reflections as $reflection)
                            <option value="{{ $reflection->id }}">{{ $reflection->title }}</option>
                        @endforeach
                    </select>
                </div>

...
@section('title', 'Onboarding - Jobb Házasság Akadémia')

@if(isset($isLoggedIn) && $isLoggedIn && Auth::guard('google')->check())
    <div class="fixed top-2 right-2 z-50 bg-white rounded-lg shadow-lg p-2 border border-gray-200 max-w-48">
        <div class="flex items-center gap-2">
            <img src="{{ Auth::guard('google')->user()->avatar_url }}" 
                 alt="Profile" 
                 class="w-6 h-6 rounded-full border border-gray-300">
            <div class="text-xs min-w-0 flex-1">
                <div class="font-medium text-gray-900 truncate">{{ Auth::guard('google')->user()->name }}</div>
            </div>
            <a href="#logout" 
               class="text-white px-2 py-1 rounded text-xs font-medium transition-colors duration-200"
               style="background-color: #467a7a;"
               onmouseover="this.style.backgroundColor='#3a6565'"
               onmouseout="this.style.backgroundColor='#467a7a'">
                Kilépés
            </a>
        </div>
    </div>
...
@section('content')
    @foreach($onsiteBlocks as $onsiteBlock)
        @php
            $typeAttributes = $onsiteBlock->type->articleTypeAttributes;
        ...
        @if($onsiteBlock->type->name === 'title')
            <h1 class="title text-center font-bold fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out" @if($style) style="{{ $style }}" @endif>@markup($onsiteBlock->content)</h1>
        @elseif($onsiteBlock->type->name === 'subtitle')
            <h5 class="subtitle text-center font-bold fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out mb-8 mt-8" @if($style) style="{{ $style }}" @endif>@markup($onsiteBlock->content)</h5>
        @elseif($onsiteBlock->type->name === 'left-subtitle')
            <h5 class="subtitle font-bold fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out text-center mb-[1rem]" @if($style) style="{{ $style }}" @endif>@markup($onsiteBlock->content)</h5>
        @elseif($onsiteBlock->type->name === 'yellow')
            <div class="transpBox fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out font-medium mt-4 mb-4" @if($style) style="{{ $style }}" @endif>
                @markup($onsiteBlock->content)
            </div>
        @elseif($onsiteBlock->type->name === 'white')
        ...
        @elseif($onsiteBlock->type->name === 'green')
        ...
        @elseif($onsiteBlock->type->name === 'image')
            <div class="boxHolder flex justify-center fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out" @if($style) style="{{ $style }}" @endif>
                <div class="imgBox w-full max-w-[400px]">
                    <img class="image w-full h-auto" src="{{ $onsiteBlock->content }}" alt="">
                </div>
            </div>
        @elseif($onsiteBlock->type->name === 'columns')
        ...
        @elseif($onsiteBlock->type->name === 'big-title')
            <h5 class="articleSubtitle font-extrabold fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out text-center mt-[1rem] mb-[1rem]" @if($style) style="{{ $style }}" @endif>@markup($onsiteBlock->content)</h5>
        @elseif($onsiteBlock->type->name === 'column-title')
            <h5 class="font-bold pt-4 text-center" @if($style) style="{{ $style }}" @endif>@markup($onsiteBlock->content)</h5>
        @elseif($onsiteBlock->type->name === 'self-awareness')
        <div class="mini-reflection fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out bg-[var(--light-butter-g)] p-2 sm:p-8 mt-4 mb-4">
                    ...
                    @foreach($reflectionQuestions as $reflectionId => $questions)
                        @foreach($questions as $question)
                            <div class="option w-full mb-10 origin-right scale-x-100 transition-transform duration-300 hover:scale-x-98">
                                <button class="flex w-full justify-between items-center dropdownButton bg-[var(--butterYellow-bg)] p-4 text-[1.2rem] text-left focus:outline-none focus:shadow-[0_0_10px_2px_rgba(0,128,0,0.7)] font-medium
                                 cursor-pointer rounded-tl-[10px] rounded-tr-[10px]">
...
<!--                                     <input type="text" placeholder="Gépelj be egy szöveget..." class="inpArea hidden bg-[var(--butterYellow-bg)] w-full p-4 transition focus:outline-none focus:outline-none cursor-pointer">
 -->                                    <textarea  class="inpArea bg-[var(--butterYellow-bg)] w-full p-4 transition focus:outline-none focus:outline-none cursor-pointer cursor-text resize-none rounded-bl-[10px] rounded-br-[10px]" 
                                            ...
                                            name="story"
                                            rows="2"
                                            placeholder="Gépelhetsz ide..."></textarea>
                                </div>
                            </div>
                    ...
                    <div class="flex justify-center">
                        <button class="btn-mini-ref bg-[#587a7b] text-white font-medium cursor-pointer px-12 py-4 rounded-[15px] hover:scale-105 transition transform shadow-[0_4px_12px_rgba(0,0,0,0.08)] hover:bg-[#328d8d] hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)]">Mentés</button>
                    </div>
                </div>
        
        </div>
        @elseif($onsiteBlock->type->name === 'quiz')
            @php
                $quizId = is_numeric($onsiteBlock->content) ? (int)$onsiteBlock->content : null;
            @endphp
            @if($quizId && isset($quizQuestions[$quizId]) && $quizQuestions[$quizId]->isNotEmpty())
                <div class="quizBox fade-section opacity-0 translate-y-5 transition-all duration-700 ease-out mt-4 mb-4" @if($style) style="{{ $style }}" @endif>
                    @foreach($quizQuestions[$quizId] as $index => $question)
                        <div class="quiz-question {{ $index === 0 ? 'active' : 'hidden' }}" data-question="{{ $index }}">
                            <h5 class="articleSubtitle font-extrabold mb-6 text-center">{{ $question->question_text }}</h5>
                            
                            @foreach($question->options as $optionIndex => $option)
                                <div class="answerBox mb-4 font-medium bg-white border-2 border-[#f5f1eb] w-full origin-right scale-x-100 transition-transform duration-300 hover:scale-x-98" 
                                     data-option-id="{{ $option->id }}" 
                                     data-is-correct="{{ $option->is_correct ? 1 : 0 }}"
                                     style="cursor: pointer !important;">
                                    <p style="font-medium">{{ $option->option_text }}</p>
                                </div>
                            @endforeach
